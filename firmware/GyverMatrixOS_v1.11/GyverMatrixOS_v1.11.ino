...
byte CLOCK_Y = byte(float(HEIGHT - 1*5) / 2 + 0.51);          // Одна строка цифр 5 пикс высотой  / 2 - в центр

// Для CLOCK_ORIENT == 1
//byte CLOCK_X = byte(float(WIDTH - (2*3 + 1)) / 2 + 0.51) ;  // 2 цифры * (шрифт 3 пикс шириной) 1 + пробел между цифрами) /2 - в центр
//byte CLOCK_Y = byte(float(HEIGHT - (2*5 + 1)) / 2 + 0.51);  // Две строки цифр 5 пикс высотой + 1 пробел между строкми / 2 - в центр
