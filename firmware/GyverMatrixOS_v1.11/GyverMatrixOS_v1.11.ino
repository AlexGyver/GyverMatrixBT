...
// Ввиду планируемой дальнейшей узкой специализации проекта вся разработка от vvip-68
// перенесена в репозиторий https://github.com/vvip-68/GyverMatrixWiFi, в котором
// Удалена поддержка
// - плат Arduino ввиду малого количество памяти, препятствующей расширению проекта
//   Платы Wemos вероятно будут работать, но не тестировались.
//   Для плат Arduino Mega вероятно потребуется переработка кода.    
// - управления по Bluetooth (оставлен только WiFi)
// - управления с кнопок
// - часов реального времени. Все часы - только из интернет c синхронизацией по NTP
// Будут добавлены:
// - Календарь в режим часов (отображение даты)
// - Будильник "рассвет"
// - Поддержка матриц большого размера (30x36), режимы переработаны под этот размер.
// - Переработана программа для Andriod-смартфонов для управления матрицей, в т.ч. будильником
